...
The dataset features 300874 distinct entries: 64207 have an unknown cause, 12301 are labelled as reignition, 222596 are undefined, and 1770 have a natural cause of ignition.

This dataset does not contain any topographical data or fuel data, except for the surrounding tree species matched with the location of the fires. It only features weather variables at the hour the fire ignited. Although the object of study is fires of natural origin, this dataset was created in order to have a historical reference for fires that occurred between 2001 and 2023 and whose cause could not be ascertained. So that, in the future, it would be possible to determine the origin of fires of uncertain origin and characterise them. From this dataset, all wildfires whose origin was only natural were extracted.
...
\label{weather_Extaction_natural_fires}
...
\label{natural_topography}
As previously stated, topography encapsulates multiple sources. The algorithms developed for extracting topographic data are similar to each other. And they work as follows: There is one iteration for each fire location and a comparison with the nearest point of data. For slope, roughness, and aspect, the data source from \cite{ESA_Sinergise_2021} came in a geotiff format, and the algorithm checked if the wildfire site was within the borders of mainland Portugal, and then it extracted the value for the nearest point. This also applied to mean elevation from \cite{Danielson2011} (this source came in a shapefile). Corine land cover code \cite{71c95a07-e296-44fc-b22b-415f42acfdf0} extraction differed a bit. It offered a json file, but for every pair of coordinates, the algorithm audited if the pair was already tabulated in the file.
The extracted values were then concatenated into the natural fire file with their respective wildfire sites.



\subsection{Creating Areas without wildfires}
In order to perform tests on models with balanced data, weather records without the occurrence of natural fires were extracted. A kind of twin fire was determined for each natural fire. The twin fire was not a fire per se, but a geographical point where no natural fire had occurred. The twin fire held the same date as the natural fire. The algorithm for the Twin Fire worked as follows: An entry from natural fires is selected. The date of the wildfire is extracted. The new event will occur on the same date as the natural fire. Then, the latitudes and longitudes of all fires within the extracted year are composed into lists. Next, the algorithm will find and generate a coordinate inside the spectrum for latitude (36.7, 42.3) and longitude (-9.8, -6) that has a distance of a minimum of 25 kilometres from all fire points from that year. This ensured that random coordinates were not equal to the ones from natural fires. The range of 25 kilometres was selected as the minimum distance between all the fires of a specific year because it is the maximum resolution described for \cite{Zippenfenig_Open-Meteo, Hersbach_ERA5, Munoz_ERA5_LAND, Schimanke_CERRA}. With \cite{OpenStreetMap} generated coordinates were geocoded to check if they belonged to Portugal and were on the mainland, far from bodies of water. The confirmation explicitly searched to see if the coordinate had an address and was inside a county. The newly generated pairs of coordinates and their respective dates were saved into a file. Subsequently, the same methods for weather record extraction with an hourly 3-month time window (\ref{weather_Extaction_natural_fires}), FWI calculation (\ref{fwi_calc_natural_fires}), and topography (\ref{natural_topography}) featured in the natural fires dataset (\ref{Natural_Fires_dataset}) were applied. At the end, the dataset without natural fires had the same number of occurrences as the dataset with natural fires.

\section{Dataset creation conclusion}
Three datasets were created. Figure \ref{fig:three_datasets} depicts the evolution of each dataset and their corresponding attributes and record associations. The first dataset served as a template for understanding the problem, how it was possible to extract meteorological data, and how to match the weather history, tree species, and the fire. The natural fires dataset comes from 1770 fires taken from the initial dataset. Topographical variables were added to this dataset, along with a longer meteorological history, and the FWI variables were calculated. Finally, in order to understand which meteorological phenomena are not associated with fire ignition, a dataset was created containing locations where no fires occurred on the same date as the fires stored in the natural fires dataset. It worked as an adversary counterpart that held the locations without an occurrence of a natural wildfire, mirroring the conditions that lead to the inexistence of a wildfire ignition. The methodologies for associating meteorological history, FWI, and topography were also applied to this dataset.
...
\begin{figure}[H]
	...
	\centering
	\includegraphics[width=\textwidth]{chapter-images/Datasetcreation/dataset_conclusion/naturalfeuer.png}
